namespace Battleship
{
	public class BattleField : ViewModelBase
	{
		...
		Cell[][] MapCell(string[] mp) //qmnis axal rukas da utolebs mas frchxilebshi gadacemul rukas
		{
			var map = new Cell[10][]; //iqmneba axali ruka
			for (int i = 0; i < 10; i++)
			{
				map[i] = new Cell[10];
				for (int j = 0; j < 10; j++)
				{
					map[i][j] = new Cell(mp[i][j], i, j); //axali map masivis yvela wevri utoldeba mp masivis yvela wevrs
				}
			}
			return map;
		}
		...
		{
			OurMap[y][x].SetState(); // tu gemi arsebobs mashin Shoot-shi chaiwereba Visible tu arada miss - shi
		}
	}
}